...
export function openProject(id: string): Project {
  ...
  if (fs.existsSync(file)) {
    try {
      ...
      console.warn(`[automatic-editing:project] ${id}: project.json is corrupt, recovering from database mirror`)
    }
  }
  project ??= row
  if (!project) {
...
export function importProjectFromFile(filePath: string): Project {
  ...
  try {
    ...
    throw new Error('That file could not be read as a project (expected a project.json).')
  }
  if (!project?.id || !project?.source || !project?.edl) {
    throw new Error('That file is not a valid Automatic Editing project (project.json).')
  }
