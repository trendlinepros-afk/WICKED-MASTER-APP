...
class RenderQueue extends EventEmitter {
  ...
  /** Cancel everything still pending/running — called on app quit so child
   *  processes (ffmpeg, HyperFrames) are killed instead of orphaned. */
  cancelAll(): void {
    for (const job of this.jobs.values()) {
      if (job.status === 'queued' || job.status === 'running') this.cancel(job.id)
    }
  }
    ...
    try {
      ...
      await next.run({
        ...
        progress: (f, detail) => {
          ...
          // Throttle progress fan-out (each emit clones + IPCs to the window);
          // status changes elsewhere always emit unthrottled.
          const now = Date.now()
