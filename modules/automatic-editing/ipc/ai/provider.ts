 ...
 *
 * Gemini/OpenAI/DeepSeek call their REST endpoints with fetch directly (no
 * SDK). Anthropic uses @anthropic-ai/sdk, imported LAZILY inside complete()
 * so the SDK never loads at shell startup.
 *
...
/**
 * Anthropic (Claude) via the official SDK (lazy-loaded). Uses the Messages
 * API — system is a top-level field (not a message), and `temperature` is
 * intentionally NOT sent (Opus 4.8 rejects it). Structured JSON is requested
 * via the prompt and parsed by the robust extractJson layer, matching how the
 * other providers are handled.
 */
export class AnthropicProvider implements AIProvider {
  ...
  async complete(req: AIRequest, signal?: AbortSignal): Promise<string> {
    // Lazy import keeps the SDK off the shell-startup path.
    ...
    try {
      ...
      if (err instanceof Anthropic.AuthenticationError || err instanceof Anthropic.PermissionDeniedError) {
        throw new Error('Anthropic rejected the API key. Check it in the shell Settings → API Keys.')
      }
