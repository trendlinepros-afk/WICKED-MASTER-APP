 ...
 * override per task in the module Settings. Falls back to the Mock provider
 * whenever the routed provider has no key in the shell vault, so keyless runs
 * still complete.
 */
...
function buildProvider(id: AIProviderId): AIProvider {
  ...
  if (!key) {
    console.warn(`[automatic-editing:ai] no ${id} key in the shell vault — falling back to mock provider`)
    return mock
  }
