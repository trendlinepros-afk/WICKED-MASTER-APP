 ...
 * NOTE: this calls the REST endpoint directly with fetch — the `openai` npm
 * package is not needed (the standalone app did the same).
 */
import fs from 'fs'
...
function mockTranscript(durationSec: number): Transcript {
  const lines = [
    'Welcome back to the channel, today we are testing Automatic Editing.',
    'This transcript is mock data because no OpenAI key is configured.',
    'Add your key in the shell Settings under API Keys to run real Whisper transcription.',
