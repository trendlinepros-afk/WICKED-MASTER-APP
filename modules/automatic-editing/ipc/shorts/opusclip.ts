...
async function opusFetch(pathname: string, init: RequestInit = {}, signal?: AbortSignal): Promise<any> {
  const key = getSecret('opusclip')
  if (!key) {
    throw new Error('No OpusClip API key. Add it in the shell Settings → API Keys (requires a Pro Beta, Max, or Business plan).')
  }
...
async function pollDetached(project: Project, state: ShortsProjectState): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      console.warn('[automatic-editing:shorts] could not persist polling error:', persistErr)
    }
  }
}
