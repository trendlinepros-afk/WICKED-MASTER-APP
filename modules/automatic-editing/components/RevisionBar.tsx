...
export default function RevisionBar() {
  ...
  async function submit() {
    ...
    setLastResult(null)
    try {
      const rev = await api.submitRevision(
        project.id,
        text.trim(),
  ...
  return (
    <div className="px-3">
      <div className={`${panel} flex items-center gap-2 p-2`}>
        <span className="text-xs text-muted shrink-0 w-44 truncate">
          {selection.region
            ...
            : selection.segmentIds.length > 0
              ...
              : 'Whole video'}
          {hasScope && (
            <button className="ml-1 text-accent hover:underline" onClick={clearSelection}>
              clear
            </button>
          )}
        </span>
        <input
          ...
          disabled={busy}
        />
        <button className={`${btnPrimary} shrink-0`} onClick={submit} disabled={busy || !text.trim()}>
          {busy ? 'Applying…' : 'Revise'}
        </button>
      </div>
      {lastResult && <p className="text-xs text-muted mt-1 px-1">{lastResult}</p>}
    </div>
  )
}
