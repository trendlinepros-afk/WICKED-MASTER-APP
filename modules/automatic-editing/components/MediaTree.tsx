...
function Node({ item, depth, numbering, videoCount = 0, onSetOrder, onRemove }: NodeProps) {
  ...
  return (
    <div>
      <div
        className="group flex items-center gap-2 rounded px-2 py-1 hover:bg-raised"
        style={{ paddingLeft: depth * 16 + 8 }}
      >
        {isFolder ? (
          <button className="w-4 text-muted shrink-0" onClick={() => setOpen((v) => !v)} aria-label={open ? 'Collapse' : 'Expand'}>
            {open ? '▾' : '▸'}
          </button>
        ) : (
        ...
        <span className={`flex-1 truncate text-sm ${numbered ? 'text-accent' : 'text-ink'}`}>{item.name}</span>

        {!isFolder && numbering && (
          <select
            className="bg-raised border border-edge rounded text-xs px-1 py-1 text-ink shrink-0"
            value={item.order ?? ''}
        ...
        {!isFolder && !numbering && numbered && (
          <span className="text-[10px] text-accent shrink-0">#{item.order}</span>
        )}
        {onRemove && (
          <button
            className="text-[11px] text-muted opacity-0 group-hover:opacity-100 hover:text-danger shrink-0"
