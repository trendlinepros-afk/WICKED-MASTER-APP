 ...
 *
 * Track colors on shell tokens: cuts=danger, transitions=warn,
 * graphics=accent, music=ok.
 */
  ...
  return (
    <div className={`${panel} p-3 select-none`}>
      ...
      <div ref={trackRef} className="relative cursor-crosshair" onMouseDown={onRulerDown}>
        {/* Ruler */}
        <div className="h-6 bg-raised rounded-t-md relative border border-edge border-b-0">
          {[...Array(11)].map((_, i) => (
            <span
              key={i}
              className="absolute top-1 text-[9px] text-muted font-mono -translate-x-1/2"
              style={{ left: `${i * 10}%` }}
            >
            ...
            return (
              <div
                ...
                className={`absolute top-0.5 bottom-0.5 rounded-sm ${
                  c.status === 'proposed' ? 'bg-danger/30 border border-danger/50' : 'bg-danger/60'
                }`}
          ...
          {project.edl.graphics
            ...
            .map((g) => (
              <div
                ...
                className={`absolute top-1 bottom-1 rounded-sm ${g.status === 'rendered' ? 'bg-accent/70' : 'bg-accent/30 border border-accent/60'}`}
        ...
        <Track label="Music" last>
          {project.edl.music.map((m) => (
            <div
              ...
              className="absolute top-1 bottom-1 bg-ok/50 rounded-sm"
        ...
        {sel && (
          <div
            className="absolute top-0 bottom-0 bg-accent/10 border-x-2 border-accent/70 pointer-events-none"
        ...
        {/* Playhead */}
        <div
          className="absolute -top-1 bottom-0 w-0.5 bg-accent pointer-events-none"
          style={{ left: pct(Math.min(playheadSrc, duration)) }}
        >
          <div className="w-2.5 h-2.5 bg-accent rotate-45 -translate-x-1 -translate-y-0.5" />
        </div>
      </div>
    </div>
  )
...
function Track({ label, children, last = false }: { label: string; children: React.ReactNode; last?: boolean }) {
  return (
    <div className={`relative h-9 bg-surface border border-edge border-b-0 ${last ? 'rounded-b-md !border-b' : ''}`}>
      <span className="absolute left-1.5 top-1 text-[9px] uppercase tracking-wider text-muted/70 pointer-events-none z-10">
        {label}
      </span>
      {children}
    </div>
...
function Edge({ onDown, side }: { onDown: () => void; side: 'left' | 'right' }) {
  return (
    <div
      className={`absolute top-0 bottom-0 w-1.5 cursor-ew-resize hover:bg-ink/40 ${side === 'left' ? 'left-0' : 'right-0'}`}
      onMouseDown={(e) => {
        e.stopPropagation()
        onDown()
      }}
