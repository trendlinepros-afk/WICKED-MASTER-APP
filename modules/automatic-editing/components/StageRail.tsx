...
import { estimatePipelineCost, formatUsd } from '../lib/cost'
...
export default function StageRail() {
  ...
  async function startPipeline() {
    ...
    if (!project.transcript) {
      const est = await api.estimateTranscription(project.id)
      setEstimate(est)
      setConfirming(true)
      return
    }
    api.runPipeline(project.id)
  }

  return (
    <aside className={`w-72 shrink-0 ${panel} p-3 flex flex-col gap-2 overflow-y-auto`}>
      <div className="flex items-center justify-between">
        <h2 className="font-semibold text-ink text-sm">Pipeline</h2>
        <button className={`${btnPrimary} text-xs`} onClick={startPipeline} disabled={anyRunning}>
          {anyRunning ? 'Running…' : 'Run pipeline'}
        </button>
      </div>

      {confirming && estimate && (
        <div className={`${panel} bg-raised p-3 text-xs space-y-2`}>
          <p className="text-ink">
            Transcription (OpenAI Whisper) will process <b>{estimate.minutes} min</b> of audio — estimated{' '}
            <b className="text-accent">${estimate.estUsd.toFixed(3)}</b>.
          </p>
        ...
        return (
          <div key={id} className={`rounded-md border p-2 ${borderCls(st.status)}`}>
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-ink flex-1">{meta.label}</span>
              <span className="text-[10px] uppercase tracking-wide text-muted">{st.status}</span>
              {(st.status === 'done' || st.status === 'stale' || st.status === 'error') && (
                <button
                  ...
                  disabled={anyRunning}
                  onClick={() => api.runStage(project.id, id)}
            ...
            <div className="flex items-baseline justify-between ml-7 mt-1">
              ...
              {cost && (
                <span className="text-[10px] shrink-0 ml-2 font-mono text-muted" title="Estimated cost per run">
                  {cost.perStage[id].local ? 'local' : `~${formatUsd(cost.perStage[id].usd)}`}
                </span>
              )}
            </div>
            {st.error && <p className="text-[11px] text-danger mt-1 ml-7">{st.error}</p>}
          </div>
        )
      })}

      {cost && (
        <div className="mt-1 pt-2 border-t border-edge flex items-baseline justify-between">
          <span className="text-xs text-ink/80">Estimated total / run</span>
          <span className="text-sm font-mono text-accent">~{formatUsd(cost.total)}</span>
        </div>
      )}
      {cost && (
